...
/*
	HELPER 
	Various predicates for data traversal, receiving children and siblings and namespace handling using nodes created by `flatten_xml/2` inside the Module `xml_flatten`
...
descendant(File, ID, Desc_ID) :-
	child(File, ID, Desc_ID).
descendant(File, ID, Desc_ID) :-
	child(File, Inter_ID, Desc_ID),
	descendant(File, ID, Inter_ID).

descendant_or_self(File, ID, ID) :-
	node(File, ID, _NS, _Name).
descendant_or_self(File, ID, Desc_ID) :-
	descendant(File, ID, Desc_ID). 
